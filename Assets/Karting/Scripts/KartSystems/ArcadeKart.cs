...
namespace KartGame.KartSystems
{
    public class ArcadeKart : MonoBehaviour
    {
        ...
        [Tooltip("VFX when speed is fast")]
        public GameObject SpeedVFX;
        [Tooltip("Accerleration sound")]
        public AudioSource AccerlerationSound;
        [Tooltip("Speed to trigger the speed VFX")]
        public float HighSpeed;
        [Tooltip("prevent repeat sound play")]
        ...
        void MoveVehicle(bool accelerate, bool brake, float turnInput)
        {
            ...
            if (GroundPercent > 0.0f)
            {
                ...
                if (Rigidbody.velocity.magnitude > HighSpeed)
                {
                    SpeedVFX.SetActive(true);
                    if (!IsPlaySound) {
                        AccerlerationSound.Play();
                        IsPlaySound = true;
                    }
